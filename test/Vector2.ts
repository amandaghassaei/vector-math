...
describe('Vector2', () => {
    ...
    it('cross() - calculate the 2D cross product with another Vector2', () => {
        ...
        // Works with threejs.
        expect(new Vector2(1, -9.2).cross(new THREE_Vector2(1.4, 0.5))).to.equal(
            13.379999999999999
        );
    });
    ...
    it('normalize() - normalizes the length of a Vector2', () => {
        ...
        expect(popLastWarning()).to.contain('Attempting to normalize zero length Vector2');
        // Check that it returns this.
    ...
    it('lerp() - linearly interpolates between two Vector3s', () => {
        ...
        // Works with threejs.
        expect(new Vector2(5.4, 0.5).lerp(new THREE_Vector2(1, 1), 0.5)).to.deep.equal(
    ...
    it('equals() - tests equality with Vector2', () => {
        ...
        expect(vector1.equals(new Vector2(5.4 + DEFAULT_NUMERICAL_TOLERANCE, 0.5))).to.equal(true);
    ...
    it('isZero() - checks if Vector2 is zero vector', () => {
        ...
        // Optional numerical tolerance.
        expect(new Vector2(DEFAULT_NUMERICAL_TOLERANCE, 0).isZero()).to.equal(true);
        expect(new Vector2(DEFAULT_NUMERICAL_TOLERANCE * 2, 0).isZero()).to.equal(false);
        expect(new Vector2(0, DEFAULT_NUMERICAL_TOLERANCE).isZero()).to.equal(true);
        expect(new Vector2(0, DEFAULT_NUMERICAL_TOLERANCE).isZero(0)).to.equal(false);
    });
