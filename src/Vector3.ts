...
export class Vector3 {
    ...
    divideScalar(scalar: number) {
        if (scalar === 0)
            console.warn(
                `Dividing by zero in Vector3.divideScalar(), stack trace: ${getStackTraceAsString()}.`
            );
    ...
    normalize() {
        ...
        if (length === 0) {
            console.warn(
                `Attempting to normalize zero length Vector3, stack trace: ${getStackTraceAsString()}.`
            );
            length = 1;
        }
